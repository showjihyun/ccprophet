...
| 분류 | 커맨드 | 기능 |
|---|---|---|
| 공통 | `ccprophet install [--dry-run]` | 훅·statusLine 등록, DB 초기화 |
| 공통 | `ccprophet ingest [--root PATH] [--file PATH]` | 과거 JSONL 백필 (one-shot) |
...
| Outcome | `ccprophet budget [<TASK_TYPE>]` | 과거 기반 예상 토큰 budget |
...
- FR-3.5: *(v0.6 new)* 세션 Replay·Compare는 CLI가 아닌 `ccprophet serve`의 Web UI에서 제공 (`/api/sessions/<sid>/replay`, `/api/pattern-diff`).
...
### 6.13 F12. Quality Watch — *killer feature* (anti-downgrade)
