...
class TestAnalyzeBloatUseCase:
    def test_returns_report_for_known_session(
        ...
        inmemory_repos.tool_defs.bulk_add(sid, [
            ToolDefBuilder().named("mcp__github").with_tokens(1400).from_source("mcp:github").build(),
    ...
    def test_execute_current_uses_latest_active(
        ...
        inmemory_repos.sessions.upsert(SessionBuilder().with_id("s1").build())
        inmemory_repos.tool_defs.bulk_add(SessionId("s1"), [
    ...
    def test_execute_current_raises_when_no_active(self, analyze_bloat: AnalyzeBloatUseCase) -> None:
        with pytest.raises(SessionNotFound):
